...
import { AparaviClient, TASK_STATE } from 'aparavi-client';
	...
	try {
		...
		for (let wait = 0; wait < 90; wait += 5) {
			...
			try {
				...
				if (status && (status.state === TASK_STATE.COMPLETED || status.state === TASK_STATE.CANCELLED)) {
					...
					if (!response) {
						const stateName = status.state === TASK_STATE.COMPLETED ? 'completed' : 'cancelled';
...
async function runTests(pdfFilePath: string) {
